...
app.get("/", function (req, res) {
  ...
  res.sendFile(__dirname + "/ex_pro_complete.html");
});
...
io.on("connection", function (socket) {
  ...
  io.emit(
    "notice",
    `${socket.id.slice(0, 5)}*****(socket.id)님이 입장하셨습니다.`
  ); // 입장한 사람에 대한 알림 메세지 전송
  update_list();
  ...
  socket.on("sendMsg", (msg) => {
    ...
    if (msg["DM"] !== "전체") {
      // (2) 귓속말 여부 체크
      let DM_socketID = Object.keys(nick_array).find(
        (key) => nick_array[key] === msg["DM"]
      );
  ...
  socket.on("disconnect", function () {
    // 접속한 사람이 퇴장했을 시에는 목록에서 제거 후, 퇴장 메세지 보내기!
